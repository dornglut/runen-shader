...
must preserve. The current realization proves ordinary valid modules, exact
source-byte preservation, Naga parse and semantic rejection, private gate
reconciliation, source-revision binding, and an actual `f16` extension construct.
It does not claim that every baseline extension named by the profile gate has
